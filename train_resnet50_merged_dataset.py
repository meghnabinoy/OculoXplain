...
DATA_DIR_CANDIDATES = [
    PROJECT_ROOT / "data" / "merged_RFMID_augmented",
    PROJECT_ROOT / "data" / "merged_RFMID",
    PROJECT_ROOT / "Data" / "merged_RFMID" / "merged_RFMID",
    PROJECT_ROOT / "Data" / "merged_RFMID",
]
ENV_DATA_DIR = os.getenv("MERGED_DATASET_DIR")
if ENV_DATA_DIR:
    DATA_DIR = Path(ENV_DATA_DIR)
else:
    DATA_DIR = next((path for path in DATA_DIR_CANDIDATES if path.exists()), DATA_DIR_CANDIDATES[0])
MODEL_SAVE_PATH = "mobilenetv2_merged_dataset_model.pth"
IMAGE_SIZE = (224, 224)
BATCH_SIZE = 16  # Reduced batch size
NUM_EPOCHS = 12
NUM_EPOCHS = int(os.getenv("TRAIN_NUM_EPOCHS", str(NUM_EPOCHS)))
LEARNING_RATE = 0.0003
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
EARLY_STOPPING_PATIENCE = 7
OVERFIT_MODE = os.getenv("TRAIN_OVERFIT_MODE", "0") == "1"
...
class MergedDataset(Dataset):
    ...
    def __init__(self, data_dir, transform=None, disease_classes=None):
        self.data_dir = Path(data_dir)
        self.transform = transform
        self.disease_classes = disease_classes if disease_classes is not None else DISEASE_CLASSES
...
class MobileNetV2Model(nn.Module):
    ...
    def __init__(self, num_classes=51):
        ...
        if not OVERFIT_MODE:
            for param in list(self.model.features.parameters())[:-10]:
...
def calculate_class_weights(dataset):
    ...
    class_counts = torch.zeros(len(dataset.disease_classes))
    ...
    if non_zero_mask.sum() == 0:
        return torch.ones(len(dataset.disease_classes))
    
    # Calculate weights only for non-zero classes
    weights = torch.ones(len(dataset.disease_classes))
    total_samples = class_counts[non_zero_mask].sum()
    
    for i in range(len(dataset.disease_classes)):
        if class_counts[i] > 0:
...
def build_weighted_sampler(train_subset):
    """Create weighted sampler for class-balanced mini-batches."""
    labels = [train_subset.dataset.labels[idx] for idx in train_subset.indices]
...
def main():
    ...
    for disease_class in DISEASE_CLASSES:
        class_dir = DATA_DIR / disease_class
        if class_dir.exists():
            image_count = len(list(class_dir.glob("*.jpg"))) + len(list(class_dir.glob("*.png")))
            if image_count > 0:
                available_classes.append(disease_class)

    if len(available_classes) < 2:
        print("[ERROR] Need at least 2 classes with images for training.")
        return

    print(f"   Classes with samples: {len(available_classes)}/{len(DISEASE_CLASSES)}")
    full_dataset = MergedDataset(DATA_DIR, transform=get_transforms(train=False), disease_classes=available_classes)
    ...
    print(f"   Classes: {len(full_dataset.disease_classes)}")
    ...
    for label_idx in np.unique(all_labels):
        class_indices = all_indices[all_labels == label_idx]
        if len(class_indices) < 2:
            train_indices.extend(class_indices.tolist())
            continue
        split_point = max(1, int(0.8 * len(class_indices)))
        np.random.shuffle(class_indices)
        train_indices.extend(class_indices[:split_point].tolist())
        val_indices.extend(class_indices[split_point:].tolist())
    ...
    print("\n[INFO] Calculating class weights for imbalanced dataset...")
    class_weights = calculate_class_weights(full_dataset).to(DEVICE)
    
    # Create model
    print(f"\n[INFO] Creating MobileNetV2 model...")
    model = MobileNetV2Model(num_classes=len(full_dataset.disease_classes)).to(DEVICE)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if val_f1_micro > best_val_f1 and not np.isnan(val_f1_micro):
            ...
            torch.save({
                ...
                'disease_classes': full_dataset.disease_classes
            }, MODEL_SAVE_PATH)
            print(f"[OK] Best model saved! (Val F1: {best_val_f1:.4f})")
        else:
            patience_counter += 1
    ...
    if len(val_preds) > 0:
        plot_confusion_matrix(val_labels, val_preds, full_dataset.disease_classes)
        ...
        try:
            ...
            class_names_present = [full_dataset.disease_classes[i] for i in unique_labels]
            ...
            print(f"Classes present in validation: {len(set(val_labels))}/{len(full_dataset.disease_classes)}")
