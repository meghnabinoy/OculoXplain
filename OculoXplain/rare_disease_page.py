...
try:
    from nlp_explainer import explain as nlp_explain
except Exception as e:
    nlp_explain = None
    NLP_IMPORT_ERROR = str(e)

if nlp_explain is None:
    try:
        ...
        if uploaded_file is not None and st.session_state.get("analyze_rare", False):
            with st.spinner("🔄 Analyzing for rare diseases..."):
                try:
                    ...
                    expl = None
                    nlp_runtime_error = None
                    try:
                        if nlp_explain is not None and len(predictions) > 0:
                        ...
                        print(f"Warning: NLP explanation failed: {e}")
                        nlp_runtime_error = str(e)
                        expl = None
                    ...
                    if expl is not None:
                        ...
                        if top_pred is not None:
                            st.markdown(
                                f"**Clinical Explanation:** Based on the retinal image, the model's top prediction is **{top_pred['name']}** with confidence **{top_pred['probability']:.1%}**."
                            )
                            st.markdown(
                                "**Patient-Friendly Explanation:** The image features are most consistent with the top predicted condition. Please use this as AI support output and confirm with an eye specialist."
                            )
                        if NLP_IMPORT_ERROR or nlp_runtime_error:
                            st.info("Advanced NLP enrichment is temporarily unavailable; showing core explanation output.")
                        else:
                            st.info("NLP backend warming up. Try analyzing once more in a few seconds.")
                        st.caption("This explanation is generated by an AI system and is not a medical diagnosis.")
